...
public class MenuOrderSummaryItem implements Serializable {
	...
	public void addPaidUser(Order order) {
		paidUsers.add(new MenuOrderSummaryUser(order.getId(), order.getUser().getUserName(), order.getUser().getDisplayName(), order.getQuantity()));
	}

	public void addUnpaidUser(Order order) {
		unpaidUsers.add(new MenuOrderSummaryUser(order.getId(), order.getUser().getUserName(), order.getUser().getDisplayName(), order.getQuantity()));
	}
