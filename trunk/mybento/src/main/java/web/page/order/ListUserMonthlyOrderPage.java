...
import web.model.UserMonthlyOrderDataProvider;
import web.page.BasePage;
import dto.YearMonthData;

@RequireUser
public class ListUserMonthlyOrderPage extends BasePage {
	public ListUserMonthlyOrderPage() {
		UserMonthlyOrderDataProvider dataProvider = new UserMonthlyOrderDataProvider(getCurrentUser().getId());
	...
	private class ViewOrderPanel extends Fragment {
		public ViewOrderPanel(String id, IModel model) {
			super(id, "view", ListUserMonthlyOrderPage.this);
			Link editLink = new Link("viewLink", model) {
				...
				public void onClick() {
					...
					setResponsePage(ViewUserMonthlyOrderPage.class, params);
				}
			};
			add(editLink);
		}
